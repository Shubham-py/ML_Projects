...
export default function PythonPractice() {
  ...
  const [openSlug, setOpenSlug] = useState<string | null>(pythonExercises[0]?.slug ?? null)

  const filtered = useMemo(
    () => (difficulty === 'All' ? pythonExercises : pythonExercises.filter((e) => e.difficulty === difficulty)),
    [difficulty],
  )
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-[var(--color-text-bright)]">Python & Pandas Practice Lab</h1>
        <p className="mt-2 text-sm text-[var(--color-text-dim)] max-w-3xl">
          Write and run real Python in your browser (numpy, pandas, and scikit-learn all available) —{' '}
          {pythonExercises.length} problems spanning algorithms, data structures, and pandas idioms actually used in
          production data science work.
        </p>
      </div>
            ...
            <div key={ex.slug} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)]">
              ...
              {open && (
                <div className="border-t border-[var(--color-border-soft)] p-4 space-y-4">
                  ...
                  <PythonPlayground starterCode={ex.starterCode} solutionCode={ex.solutionCode} />
